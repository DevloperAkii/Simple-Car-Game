...
function Game() {
  //Create Road Rect
  const roadRect = new Graphics()
    .rect(app.screen.width / 2, 0, 500, app.screen.height)
    .fill("black");
  ...
  for (let i = 0; i < 20; i++) {
    const roadLineRect = new Graphics()
      .rect(0, app.screen.height - 50, 25, 50)
      .fill("white");
  ...
  const rightButton = new Graphics()
    .rect(app.screen.width / 2 - 1, 0, roadRect.width / 2, app.screen.height)
  ...
  app.ticker.add((time) => {
    for (let i = 0; i < roadLines.children.length; i++) {
      ...
      if (roadLines.children[i].position.y > app.screen.height) {
        roadLines.children[i].position.y =
          roadLines.children[roadLines.children.length - 1].position.y - 200;
